...
所以 `voice_direction.reference_ref` 以参考音频为主载体。没有参考音频时保持
`reference_ref: null` 并写明「待选型」——**不要用一段文字描述冒充音色身份**：下游拿着
一段形容词去找声音，找回来的每次都不一样，而记录看起来是完整的。

界线判据一句话：**这个特征在角色安静地坐着说一句无关紧要的话时还在吗？** 在，就是身份；
不在，就是表演，留给场次与台词。界线画错的代价不对称：把表演写进身份，角色每一集都会
被要求带着上一集的伤势说话；把身份漏进表演，配音者每一场都要重新猜这个人是谁。
