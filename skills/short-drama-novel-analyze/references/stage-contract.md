...
本文件只定义本阶段的所有权、材料边界与规则。公共安装校验、事务恢复和状态读取由 core
`preflight`、`prepare` 处理，不要从子技能重新解析 suite manifest。

## 阶段入口补充

- Agent 与索引脚本只写 `项目开发/source-analysis/_work/` 工作文件；校验通过后再由
  `finalize --publish` 发布，不直接覆盖正式分析产物。
- `输入/` 下原始材料按整文件 hash 绑定。原文变化后重新生成 `_index.json` 及全部下游分析；
  不沿用旧 span。
