...
|---|---|---|
| “根据这个点子写第一集” | write, or develop if creator asks for alternatives | do not force series planning; 设定集有记录则先读作约束 |
| “先把主角和主场景的设定定下来” | assets 播种模式 | 从 develop 产物或创作者描述写身份层 proposed 记录；不补造剧本证据 |
...
| “检查这集有没有模板感/AI 味” | 冷读审查（类型基准未立时 fresh review） | 诊断模板感，只引用位置、损失和 owner，不改正文 |
| “直接把这集去 AI 味并保留人物口吻” | write | 做定点修订与语义 diff，不自称独立审查通过 |
| “先检查模板感，再按问题改掉并复查” | fresh/cold_read 审查 → write → delta_verify 增量复核 | 冻结旧 hash；finding 路由给 owner；新 hash 在当前上下文按 base 结论逐条核销，越界或交付前回到 fresh reviewer |
