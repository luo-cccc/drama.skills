...
| 技能 | 职责 |
|---|---|
| `short-drama` | 项目管理（需显式调用）：初始化 / 恢复 / 状态 / 交付打包、本地创作台 Dashboard、制作形态与视觉方向决定、生命周期工具（`project_tool.py`） |
...
本套件严格按显式调用工作。当前请求没有写出对应的 `$short-drama` 或
`$short-drama-*` 技能名时，不得自动触发任何技能。显式调用子技能后，再从用户指定路径
或当前目录向上查找 `short-drama.json`；找不到时停止并提示用户显式调用 `$short-drama`
初始化，不能自动代为调用。项目已初始化后，用户仍须点名本次要使用的具体技能。
剧本引入设定集之外的新元素时，
...
输入的结构化 refs 自动收集 JSONL `record_id` 与 JSON 字段指针，一个 8 角色的文件不用手写
8 条 `--input-record`——设定集追加新身份不会让引用它的产物连锁 `stale`。所有 canonical
selector 无论是否自动收窄都必须真实且唯一解析；不存在的记录、字段或不支持记录定位的
Markdown 会直接拒绝发布。新版剧本索引使用
